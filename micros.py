...
DEFAULT_SAVE    = os.path.join(SAVE_DIR, "micros.json")
...
class SpreadsheetWidget(tk.Frame):
    ...
    def _bind_keys(self):
        for seq, fn in [
            ...
            ("<Tab>",    self._on_tab),
            ("<Shift-Tab>", self._on_shifttab),
            ("<ISO_Left_Tab>", self._on_shifttab),
    ...
    def _ensure_visible(self, r, c):
        ...
        if x1 < cx1 + HEADER_W:
            self.canvas.xview_moveto((x1 - HEADER_W) / (self.cols * CELL_W + HEADER_W))
        elif x2 > cx2:
            self.canvas.xview_moveto((x2 - self.canvas.winfo_width()) / (self.cols * CELL_W + HEADER_W))
            
        if y1 < cy1 + COL_HEADER_H:
            self.canvas.yview_moveto((y1 - COL_HEADER_H) / (self.canvas.bbox("all")[3]))
        elif y2 > cy2:
            self.canvas.yview_moveto((y2 - self.canvas.winfo_height()) / (self.canvas.bbox("all")[3]))

    def _on_enter(self):
        if not self.sel_start: return
        r, c = self.sel_start
        # No Excel, Enter inicia edição se não estiver editando,
        # ou move para baixo se já estiver editando (aqui o Enter da entry já committa)
